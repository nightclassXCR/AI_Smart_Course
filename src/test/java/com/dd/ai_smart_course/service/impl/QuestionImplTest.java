...
class QuestionImplTest {
    ...
    void testCreateQuestion_Success() {
        ...
        when(optionMapper.insertBatch(anyList())).thenReturn(4);

        // When
        assertDoesNotThrow(() -> questionService.createQuestion(validQuestionDTO));

        // Then
        verify(questionMapper, times(1)).insert(any(Question.class));
        verify(optionMapper, times(1)).insertBatch(anyList());
    }
    ...
    @Test
    void testCreateQuestion_WithEmptyContent() {
        // Given
        validQuestionDTO.setContent("");

        // When & Then
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class,
                () -> questionService.createQuestion(validQuestionDTO));
    ...
    @Test
    void testCreateQuestion_WithInvalidCourseId() {
        // Given
        validQuestionDTO.setCourseId(0);
    ...
    @Test
    void testCreateQuestion_WithInvalidChapterId() {
        // Given
        validQuestionDTO.setChapterId(-1);

        // When & Then
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class,
                () -> questionService.createQuestion(validQuestionDTO));

        assertEquals("Chapter ID must be positive", exception.getMessage());
        verify(questionMapper, never()).insert(any(Question.class));
    }

    @Test
    void testCreateQuestion_WithEmptyOptions() {
        // Given
        ...
        // Then
        verify(questionMapper, times(1)).insert(any(Question.class));
        verify(optionMapper, never()).insertBatch(anyList());
    }

    @Test
    void testCreateQuestion_WithNullOptions() {
        // Given
        validQuestionDTO.setOptions(null);
        when(questionMapper.insert(any(Question.class))).thenReturn(1);
    ...
    void testUpdateQuestion_Success() {
        ...
        when(questionMapper.update(any(Question.class))).thenReturn(1);
        when(optionMapper.deleteByQuestionId(1)).thenReturn(4);
        when(optionMapper.insertBatch(anyList())).thenReturn(4);

        // When
        assertDoesNotThrow(() -> questionService.updateQuestion(validQuestionDTO));

        // Then
        verify(questionMapper, times(1)).findById(1);
        verify(questionMapper, times(1)).update(any(Question.class));
    ...
    void testUpdateQuestion_QuestionNotFound() {
        // Given
        when(questionMapper.findById(1)).thenReturn(null);

        // When & Then
        IllegalArgumentException exception = assertThrows(IllegalArgumentException.class,
                () -> questionService.updateQuestion(validQuestionDTO));

        assertEquals("Question not found with id: 1", exception.getMessage());
        verify(questionMapper, times(1)).findById(1);
        verify(questionMapper, never()).update(any(Question.class));
    }
    ...
    @Test
    void testCreateQuestion_MapperException() {
        // Given
        when(questionMapper.insert(any(Question.class))).thenThrow(new RuntimeException("数据库连接失败"));
    ...
    @Test
    void testUpdateQuestion_MapperException() {
        // Given
        when(questionMapper.findById(1)).thenReturn(validQuestion);
        when(questionMapper.update(any(Question.class))).thenThrow(new RuntimeException("更新失败"));
